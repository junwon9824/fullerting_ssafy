# 1. Deployment: 애플리케이션의 Pod(컨테이너 묶음)를 관리합니다.
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  replicas: 2 # 2개의 Pod를 실행하도록 설정 (조정 가능)
  selector:
    matchLabels:
      app: fullerting-backend
  template:
    ...
    spec:
      containers:
        - name: backend
          # build 잡에서 생성한 Docker 이미지 주소를 사용합니다.
          image: ghcr.io/OWNER/REPO/backend:latest # 이 부분은 워크플로우에서 실제 저장소 경로로 자동 변경됩니다.
          ports:
...
spec:
  type: ClusterIP # 클러스터 내부에서만 접근 가능하도록 변경 (Ingress가 외부 접근을 처리)
  selector:
    app: fullerting-backend # 위 Deployment의 Pod를 타겟으로 합니다.
  ports:
    - protocol: TCP
      port: 8080 # 서비스가 클러스터 내부에서 사용할 포트
      targetPort: 8080 # Pod의 컨테이너가 리스닝하는 포트
      # nodePort: 30007 # (선택사항) 외부에서 접근할 노드의 포트. 지정하지 않으면 자동 할당됩니다.
