...
#### ? 메시지 키를 활용한 순서 보장 및 병렬 처리

본 프로젝트의 실시간 경매 시스템에서는 **메시지 키(Message Key)** 를 전략적으로 사용하여 데이터 정합성과 처리 성능을 동시에 확보했습니다.

1.  **동일 경매 건의 순서 보장**

    - 입찰 요청 메시지를 Kafka Producer가 전송할 때, **게시글 ID(`exArticleId`)를 메시지 키로 사용**합니다.
    - Kafka는 동일한 키를 가진 메시지들을 항상 **동일한 파티션**으로 전송하는 것을 보장합니다.
    - 결과적으로, 하나의 경매에 대한 모든 입찰(예: 100원 입찰 → 200원 입찰)은 파티션 내에서 순서대로 처리되어, 낮은 가격의 입찰이 높은 가격의 입찰을 덮어쓰는 문제를 원천적으로 방지합니다.
    ...
    public void sendBidRequest(Long exArticleId, int dealCurPrice, MemberProfile bidder) {
        ...
        // exArticleId를 Key로 사용하여 동일 경매 건은 동일 파티션으로 전송
        bidRequestKafkaTemplate.send(BID_TOPIC, exArticleId.toString(), bidRequest);
    }
    ```

2.  **서로 다른 경매 건의 병렬 처리**
    - 서로 다른 경매(다른 `exArticleId`)는 다른 파티션에 할당될 수 있습니다.
    - 컨슈머 그룹의 여러 컨슈머(또는 `concurrency` 설정에 따른 스레드)가 각기 다른 파티션을 동시에 처리하므로, 전체 시스템의 처리량(Throughput)이 극대화됩니다.
